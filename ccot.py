...
def main():
    ...
    elif args.method == "contrast_cot":
        demo = create_demo_text(args, cot_flag=True, is_contrast=True)
    else:
        pass
    ...
    for i, data in enumerate(dataloader):
        ...
        z = decoder.decode(args, x, max_length, i, sample_n)   # response.choices

        for j in range(len(z)): # 生成多个回答
...
def parse_arguments():
    ...
    parser.add_argument(
        "--model", type=str, default="gpt3", choices=["gpt3", "gpt3-medium", "gpt3-large", "gpt3-xl", "gpt3.5-turbo"],
        help="model used for decoding. Note that 'gpt3' are the smallest models."
    )

    parser.add_argument(
        "--method", type=str, default="contrast_cot",
        choices=["zero_shot", "zero_shot_cot", "few_shot", "few_shot_cot", "contrast_cot", "random"], help="method"
    )
    parser.add_argument(
